            ...
            <div class="ibox float-e-margins">
                <div class="ibox-content">
                    ...
                    <form class="form-inline" id="search_from" name="search_from">
                        ...
                        <button  type="button" onclick="findBook()" id="search" class="btn btn-warning"><span class="glyphicon glyphicon-search" aria-hidden="true"></span> 查询</button>
                    </form>
...
<script type="text/javascript">
    ...
    var borrowlist= {
        //获取用户id
        id:'<@shiro.principal type="com.bookmanage.base.entity.User" property="id"/>',
        booklist:booklist,
        //添加图书
            ...
            columns: [
                {
                    ...
                    formatter: function (value, row, index) {
                        var operateHtml = '<button type="button" class="btn btn-primary btn-sm addbtn" onclick="addReserve(\''+row.bookId+'\',\''+row.bookName+'\',\''+row.bookAuthor+'\',\''+row.bookPress+'\')"> <span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 加入预定购物车</button>';
                        return operateHtml;
                    }
                }]
        });
    ...
    function addReserve(bookid,bookName,bookAuthor,bookPress) {
        ...
        if(isInArry==0)
        {
            ...
            newLine += "<td>"+bookName+"</td>";
            newLine += "<td>"+bookAuthor+"</td>";
            newLine += "<td>"+bookPress+"</td>";
            newLine += "<td><button id=delete class='btn btn-xs btn-danger'><span class='glyphicon glyphicon-remove' aria-hidden='true'><span> 删除</td>";
            newLine += "</tr>";
